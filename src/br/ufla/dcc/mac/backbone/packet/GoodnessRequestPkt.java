...
public class GoodnessRequestPkt extends WlanFramePacket {
	...
	public GoodnessRequestPkt(Address sender, NodeId receiver,
			ElectorAgent agent) {
		this(sender, receiver, agent, PacketType.CONTROL,
				DEFAULT_SIGNAL_STRENGTH);
	}

	public GoodnessRequestPkt(Address sender, NodeId receiver,
			ElectorAgent agent, PacketType type, double signalStrength) {
		super(sender, receiver, type, signalStrength);
		_agent = agent;
	}

	public GoodnessPkt evaluate(Node node) {
		return new GoodnessPkt(node.getLayer(LayerType.MAC).getSender(),
				NodeId.ALLNODES, _agent.evaluate(node), _agent);
	}
